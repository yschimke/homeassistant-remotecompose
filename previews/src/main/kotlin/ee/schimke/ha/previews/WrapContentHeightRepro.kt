 ...
 * **Setup**
 *
 * - One `Profile` ("wrapProfile") that bakes `Header.FEATURE_PAINT_MEASURE = 0` so the player runs
 *   a real measure pass against the parent's max constraints.
 * - `RemoteComposePlayerFlags.shouldPlayerWrapContentSize = true`, so the player's modifier becomes
 *   `wrapContentSize()` instead of `Modifier.size(documentWidth.dp, documentHeight.dp)`.
 * - Document content is a `RemoteColumn(fillMaxWidth)` with a fixed-height `RemoteText` inside (≈25
 *   dp tall).
 *
 * **Expected**
 *
 * With the wrap profile + wrap-content flag + a host modifier of `Modifier.width(180.dp)` (no
 * height), the player View should report ~25 dp as its measured height up to Compose, so the slot
 * shrinks to ~25 dp. That is what `RemoteComposePlayerFlags.kt` and the comment on
 * `Header.FEATURE_PAINT_MEASURE` advertise.
 *
 * **Actual**
 *
 * The slot stretches to the parent's full available height. The document itself does measure to ~25
 * dp internally — visible if you wrap content in a bordered `RemoteBox(fillMaxWidth)` and watch
 * where the border ends — but the View does not propagate that intrinsic height to its
 * `AndroidView` parent in Compose.
 *
 ...
 * `androidx.compose.compiler:compiler:1.5.x` (Robolectric Compose preview pipeline; same behaviour
 * observed in real Activity hosts).
 *
 * Three side-by-side hosts in this preview:
 * 1. **width pinned**: `Modifier.width(180.dp)` (no height) — height should adapt; in alpha010 the
 *    slot fills available height.
 * 2. **width + height pinned**: `Modifier.width(180.dp).height(40.dp)` — control case; works
 *    because the EXACTLY constraint dominates.
 * 3. **wrap both**: `Modifier` (no width / height) — same failure mode as (1).
 *
 * Each card draws a 1.dp red border on its host (Compose UI side) and a 1.rdp blue border around an
 * outer `RemoteBox(fillMaxWidth)` that follows the document's intrinsic content height. When wrap
 * works, blue and red coincide; when it doesn't, blue sits inside a much taller red rectangle.
 */
...
@Preview(
  name = "wrap-content height repro · alpha010",
  widthDp = 411,
  heightDp = 600,
  showBackground = true,
...
fun WrapContentHeightReproAlpha010() {
  ...
  Column(modifier = Modifier.fillMaxWidth().padding(8.dp)) {
    ...
    Text(
      text =
        "Red = host slot (Compose). Blue = document's intrinsic content height. " +
    ...
    Row(modifier = Modifier.fillMaxWidth().border(1.dp, Color(0xFF2E7D32))) {
      ReproPlayer(modifier = Modifier.width(180.dp).height(40.dp).border(1.dp, Color(0xFFD32F2F)))
    }
  }
}
 ...
 * [WrapAdaptiveRemoteDocumentPlayer], which pre-measures the captured document with a real
 * `RemoteContext` (1×1 throwaway canvas to seed the paint context) and pins the host `Box` to the
 * resulting intrinsic dimensions before delegating to the upstream `RemoteDocumentPlayer` at
 * EXACTLY constraints. Result: the host slot tracks the document's intrinsic content height even
 * when only width is constrained.
 */
@Preview(
  name = "wrap-content height fix · WrapAdaptive player",
  widthDp = 411,
  heightDp = 300,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
