 ...
 * With `LocalRcDebugBorders` provided as `true`:
 *
 * - the host `Box` draws a red 1.dp border (Compose UI side)
 * - the captured document draws a blue 1.rdp border around an outer wrapper that follows its
 *   intrinsic content height (RemoteCompose side, see [DebugRcBorderWrapper])
 *
 * Mismatch readings:
 *
 * - **Exact**: red and blue coincide → host slot height matches the document's content height.
 * - **Slack** (red taller than blue): host slot is taller than the document's content; the document
 *   is anchored to the top with transparent padding below.
 * - **Tight** (red shorter than blue): host slot is shorter than the document's content; with
 *   `paint-measure` the player still paints the doc into its declared rect (clipping at the slot);
 *   with the wrap-friendly profile the player measures against the parent's `maxHeight` and the doc
 *   shrinks to fit.
 *
 * Two profile variants:
 *
 * - `androidXExperimental` — sets `FEATURE_PAINT_MEASURE = 1` (the alpha default). The player
 *   reports the document's declared size to its parent rather than the layout's intrinsic size, so
 *   a `wrapContentHeight()` host can't shrink to content.
 * - `androidXExperimentalWrap` — sets `FEATURE_PAINT_MEASURE = 0`, so the player can run a real
 *   measure pass and report intrinsic content size up to the host. Required (along with
 *   `RemoteComposePlayerFlags.shouldPlayerWrapContentSize = true` on the player side) for adaptive
 *   wrap-content slot sizing.
 *
 * Renders both 411-dp-wide (mobile) and a wider canvas so dashboard stacking is visible at typical
 * content widths.
 */
...
@Preview(
  name = "sizing — fixed slot, paint-measure profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
...
@Preview(
  name = "sizing — slack slot, paint-measure profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — tight slot, paint-measure profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — fixed slot, wrap profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — slack slot, wrap profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — tight slot, wrap profile",
  widthDp = EXP_WIDTH,
  heightDp = EXP_HEIGHT,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — width × height matrix · wrap profile",
  widthDp = 800,
  heightDp = 1300,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — width × height matrix · paint-measure profile",
  widthDp = 800,
  heightDp = 1300,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Preview(
  name = "sizing — width pinned, height adaptive",
  widthDp = EXP_WIDTH,
  heightDp = 800,
  showBackground = true,
  backgroundColor = 0xFFFFFFFFL,
...
@Composable
private fun DebugStack(title: String, profile: Profile, slotHeightStrategy: SlotHeightStrategy) {
...
private fun ExperimentRow(
    ...
    when (slotHeightStrategy) {
      ...
      is SlotHeightStrategy.NaturalDpPlus ->
        Modifier.fillMaxWidth()
          .height((kind.baselineHeightDp + slotHeightStrategy.delta).coerceAtLeast(8).dp) to
          "host=${kind.baselineHeightDp + slotHeightStrategy.delta}dp"
    }
  Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
    ...
    Box(modifier = Modifier.fillMaxWidth().border(1.dp, Color(0xFF2E7D32)).padding(1.dp)) {
      CachedCardPreview(
...
/**
 * Stacks one tile + one entities card with `Modifier.width(180.dp)` and **no** height modifier —
 * the host slot is asked to shrink to the document's intrinsic content height. Renders both
 ...
 * blue border. That's why the dashboard still pins `Modifier.height(naturalHeightDp.dp)` rather
...
private fun ConstraintRow(
  ...
  val (slotMod, heightLabel) =
    when {
      ...
      variant.heightDp != null -> widthMod.height(variant.heightDp.dp) to "${variant.heightDp}dp"
...
private fun experimentCards(): List<Triple<String, CardConfig, CardKind>> =
  listOf(
    ...
    Triple("entity", card("""{"type":"entity","entity":"sensor.living_room"}"""), CardKind.Entity),
    Triple(
      "entities",
      card(
        """{"type":"entities","title":"Living Room","entities":[
