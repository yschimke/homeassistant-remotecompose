 ...
 *
 * The card has no HA reference capture (HA's frontend doesn't ship a native garage card — this is
 * our thing), so the canvas is tight to content per [CardPreviews] rule 3.
 *
 * **Animation strip** — [Garage_Animation_Light] / [Garage_Animation_Dark] fan out via
 * [GarageDoorPositionsProvider] across the five-frame 0% / 25% / 50% / 75% / 100% open cycle.
 * Reading the resulting PNGs in order is how we verify the door rises smoothly: groove spacing
 * compresses, motion arrow stays centred, frame doesn't clip at any intermediate height.
 *
 * **State strip** — [Garage_State_Light] / [Garage_State_Dark] fan out via
 * [GarageDoorStatesProvider] over closed / opening / open / closing / unavailable so each branch of
 * the converter's state-derivation produces a frame.
 *
 * **Two-entity** — [Garage_TwoEntities_Light] / `_Dark` covers the card-with-title + multi-row
 * layout (one door open, one closed) the shutter card preview already covers for windows.
 */
...
) = GarageHost(HaTheme.Light) { RenderChild(garageSingleCard(), param.second) }
...
fun Garage_Animation_Dark(
  @PreviewParameter(GarageDoorPositionsProvider::class) param: Pair<String, HaSnapshot>
) = GarageHost(HaTheme.Dark) { RenderChild(garageSingleCard(), param.second) }
...
@Composable
fun Garage_TwoEntities_Light() =
  GarageHost(HaTheme.Light) { RenderChild(garageDoubleCard(), garageDoubleSnapshot()) }

@Preview(name = "garage two-entities (dark)", showBackground = false, widthDp = 280, heightDp = 340)
@Composable
fun Garage_TwoEntities_Dark() =
  GarageHost(HaTheme.Dark) { RenderChild(garageDoubleCard(), garageDoubleSnapshot()) }

private fun garageSingleCard() =
  card(
    """{
...
private fun garageDoubleSnapshot(): HaSnapshot =
  snapshot(
    "cover.garage_main" to
      ee.schimke.ha.model.EntityState(
