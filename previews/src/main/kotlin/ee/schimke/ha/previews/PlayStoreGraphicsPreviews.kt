...
/**
 * Play Store listing graphics rendered from the same adaptive-icon resources
 * (`ic_launcher_foreground` / `ic_launcher_background`) the launcher uses, so a recolour of the
 * brand only needs editing in one place.
 *
 * - **Icon**: 512×512 PNG, opaque background, no transparency. Drawn as a full-bleed background
 *   square with the foreground vector centred at native size — this matches the in-app icon as
 *   users see it on a square launcher (no adaptive masking, no per-OEM safe-zone clip).
 * - **Feature graphic**: 1024×500 PNG, opaque background, no transparency. Banner laid out as
 *   icon + wordmark + tagline.
 *
 * Device dpi is fixed at 320 (2.0x density) so widthDp / heightDp map cleanly to the required pixel
 * dimensions: 256dp×256dp → 512×512, 512dp×250dp → 1024×500.
 *
 * Rendered by `:previews:renderAllPreviews`; the `scripts/render-play-assets.sh` helper copies the
 * PNGs into `app/src/main/play/listings/en-GB/graphics/{icon,feature-graphic}/`.
 */
...
fun Play_Icon_512() {
  ...
  Box(
    modifier =
      Modifier.fillMaxSize().background(colorResource(ComponentsR.color.ic_launcher_background)),
