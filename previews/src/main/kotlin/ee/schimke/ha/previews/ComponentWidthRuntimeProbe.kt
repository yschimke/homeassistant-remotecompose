...
/**
 * Runtime probe comparing the inline `RemoteStateLayout(RemoteBoolean)` path (verified in earlier
 * probe iterations) against the `RemoteSizeBreakpoint` helper at the same five canvas widths. Top
 * row runs the inline path with a hand-rolled boolean state layout; bottom row drives the helper
 * with a 120 dp threshold (≈ 315 px at 2.625 density).
 */
@Preview(
  name = "componentWidth runtime probe",
...
private fun InlineCell(widthDp: Int, heightDp: Int) {
  LabelledRcCell(
    label = "in ${widthDp}×${heightDp}",
    widthDp,
    heightDp,
    ProbeKey("in", widthDp, heightDp),
  ) {
    val width =
    ...
    RemoteStateLayout(isLarge) { on ->
      RemoteText(text = if (on) "INLINE-ON".rs else "INLINE-OFF".rs, color = Color.White.rc)
    }
  }
}
...
private fun HelperCell(widthDp: Int, heightDp: Int) {
  LabelledRcCell(
    label = "helper ${widthDp}×${heightDp}",
    widthDp,
    heightDp,
    ProbeKey("hp", widthDp, heightDp),
  ) {
    RemoteSizeBreakpoint(
      thresholdsDp = intArrayOf(120),
      modifier = RemoteModifier.rcFillMaxSize(),
    ) { tier ->
      RemoteText(text = if (tier == 0) "HELPER-T0".rs else "HELPER-T1".rs, color = Color.White.rc)
    }
  }
}
...
private fun LabelledRcCell(
  ...
  Column(modifier = Modifier.width(widthDp.dp), verticalArrangement = Arrangement.spacedBy(2.dp)) {
    ...
    Box(modifier = Modifier.size(widthDp.dp, heightDp.dp).border(1.dp, HaTheme.Dark.divider)) {
      CachedCardPreview(
        cacheKey = key,
