...
/**
 * Responsive tolerance previews using realistic widths a dashboard commonly hits: narrow phone
 * column, standard mobile width, and tablet pane.
 */
@Preview(name = "entities responsive narrow", widthDp = 260, heightDp = 180, showBackground = false)
@Preview(
  name = "entities responsive standard",
  widthDp = 381,
  heightDp = 180,
...
@Composable
fun Entities_Responsive_Light() =
  CardHost(HaTheme.Light) {
    RenderChild(
      card(
        """{"type":"entities","title":"Living Room and Kitchen Devices with Very Long Heading","entities":["sensor.living_room","light.kitchen","switch.coffee_maker"]}"""
      ),
      Fixtures.mixed,
...
@Composable
fun Glance_Responsive_Dark() =
  CardHost(HaTheme.Dark) {
    RenderChild(
      card(
        """{"type":"glance","title":"Overview and Alerting Summary for Main Floor","entities":["sensor.living_room","light.kitchen","lock.front_door"]}"""
      ),
      Fixtures.mixed,
      RemoteModifier.fillMaxWidth(),
    )
...
@Preview(
  name = "markdown responsive standard",
  widthDp = 381,
  heightDp = 140,
  showBackground = false,
)
@Composable
fun Markdown_Responsive_Light() =
  CardHost(HaTheme.Light) {
    RenderChild(
      card(
        """{"type":"markdown","title":"Notes for This Evening and Tomorrow","content":"Welcome home.\n- Dishwasher running\n- Back door locked"}"""
      ),
      Fixtures.mixed,
      RemoteModifier.fillMaxWidth(),
    )
