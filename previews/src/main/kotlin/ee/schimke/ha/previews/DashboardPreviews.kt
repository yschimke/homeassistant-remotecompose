...
/**
 * Whole-dashboard previews — one column per card, stacked vertically. Used to compare
 * adaptive-layout candidates: render each captured dashboard at mobile (411 dp wide) and tablet
 * (800 dp wide) and look at the resulting PNGs side-by-side.
 *
 * Card heights flow from each document's intrinsic content via `WrapAdaptiveRemoteDocumentPlayer`
 * (used by `CachedCardPreview`), so vertical-stack / grid / sections cards size themselves to
 * whatever they actually drew. The whole dashboard renders inside one `verticalScroll` so previews
 * longer than the canvas crop instead of clipping silently.
 *
 * Skips rendering when the sibling [homeassistant-agents] checkout isn't present at the expected
 * path (see [DashboardFixtures]). The preview lambda short-circuits with a single placeholder
 * string in that case so CI runs of the previews module don't fail.
 */
...
@Preview(name = "dashboard 3d-printing — mobile", widthDp = MOBILE_WIDTH, heightDp = MOBILE_HEIGHT)
@Composable
fun Dashboard3dPrintingMobile() = DashboardPreview("3d_printing")

@Preview(name = "dashboard 3d-printing — tablet", widthDp = TABLET_WIDTH, heightDp = TABLET_HEIGHT)
@Composable
...
private fun DashboardPreview(name: String) {
  ...
  Column(
    ...
    loaded.dashboard.views.forEach { view -> ViewBlock(view, loaded.snapshot, bitmapLoader) }
  }
}

/**
 * Width threshold below which we collapse the sections grid to a single column — matches HA's web
 * frontend behaviour, which packs sections only on viewports wide enough that one section keeps a
 * reasonable per-card width (~380 dp).
 */
private const val SECTIONS_GRID_MIN_WIDTH_DP = 600

/**
 * Per-column target width on tablet+. Sections fill `max_columns` columns up to whatever fits in
 * the canvas at this width.
 */
...
/**
 * `type: sections` view layout. On viewports below [SECTIONS_GRID_MIN_WIDTH_DP] we render
 * single-column (mobile path); on wider hosts we pack sections into N columns based on the canvas
 * width and the view's `max_columns:` config (default 4 per HA).
 *
 * Sections are distributed by walking left-to-right through columns, appending to the column with
 * the smallest current card-count — keeps column heights roughly balanced without honouring
 * `column_span` / `row_span` yet (those are minor v2 polish).
 */
@Composable
...
/**
 * Greedy pack — append each section to the shortest column. Keeps column heights balanced without
 * modelling per-card pixel heights.
 */
  ...
  Box(
    ...
    CachedCardPreview(
      ...
      ProvideCardRegistry(registry) {
        ProvideHaTheme(HaTheme.Light) { RenderChild(card, snapshot, RemoteModifier.fillMaxWidth()) }
      }
    }
  }
}
