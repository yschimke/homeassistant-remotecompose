...
/**
 * Image samples — exercise the three ways RemoteCompose carries images into a `.rc` document:
 *
 * - `RemoteHaImageInline` — anonymous bytes baked in. Renders identically in `RemotePreview` (no
 *   loader needed).
 * - `RemoteHaImageNamed` — bytes baked in with a registry name. Renders without a loader; the name
 *   is the channel for state updates.
 * - `RemoteHaImageUrl` — URL only. The player calls `BitmapLoader.loadBitmap(url)` at playback.
 *   Hosts **must** wire a loader (`RemotePreview`'s default `AndroidBitmapLoader` calls
 *   `URL.openStream()` and crashes in offline / preview contexts); use [HaEmbeddedPlayer] + a fake
 *   `CoilBitmapLoader` (see [previewCoilBitmapLoader]) in previews / tests.
 *
 * Bitmaps are programmatically drawn so renders don't depend on drawable resources.
 */
...
/**
 * Distinct from [sampleBitmap] so the URL fake-coil preview can prove the loader actually resolved
 * (vs. a default fallback).
 */
