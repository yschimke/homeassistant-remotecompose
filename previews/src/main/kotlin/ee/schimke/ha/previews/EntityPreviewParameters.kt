...
class KitchenLightStatesProvider : PreviewParameterProvider<Pair<String, HaSnapshot>> {
  override val values: Sequence<Pair<String, HaSnapshot>> =
    sequenceOf(
      "on" to
        snapshot(
          state("light.kitchen", "on", mapOf("friendly_name" to "Kitchen", "brightness" to "200"))
        ),
      "off" to snapshot(state("light.kitchen", "off", mapOf("friendly_name" to "Kitchen"))),
      "unavailable" to
        snapshot(state("light.kitchen", "unavailable", mapOf("friendly_name" to "Kitchen"))),
    )
}
...
/**
 * Animation storyboard for the garage door card: a single entity with synthesised
 * `current_position` values stepping from 0 (fully closed) to 100 (fully open) in 25-point
 * increments. Combined with a `@Preview` fan-out this produces five PNGs that visually reconstruct
 * the open cycle so a reviewer can scan the strip and spot rendering glitches at intermediate
 * frames — clipping at the groove transitions, frame-overflow, motion arrow alignment.
 *
 * The intermediate positions also exercise `state: "opening"` so the motion arrow renders. End
 * frames use `closed` / `open` so the arrow is suppressed at rest.
 */
...
/**
 * Coarse states for a garage cover that doesn't expose `current_position`. Five-way fan-out
 * exercises every branch of the converter's state-derivation: closed → 100% closed; opening /
 * closing → halfway with the motion arrow; open → fully open; unavailable → defaults to fully
 * closed with an "Unavailable" label.
 */
...
class FrontDoorLockStatesProvider : PreviewParameterProvider<Pair<String, HaSnapshot>> {
  override val values: Sequence<Pair<String, HaSnapshot>> =
    sequenceOf(
      "locked" to
        snapshot(state("lock.front_door", "locked", mapOf("friendly_name" to "Front door"))),
      "unlocked" to
        snapshot(state("lock.front_door", "unlocked", mapOf("friendly_name" to "Front door"))),
      "locking" to
        snapshot(state("lock.front_door", "locking", mapOf("friendly_name" to "Front door"))),
    )
}
