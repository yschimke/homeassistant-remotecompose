 ...
 * so the resulting `.rc` document is played inside an ordinary Compose layout. The host:
 *
 ...
 * dispatch must drop to Tier-1 (use `RemoteHa*` directly inside their own
 * `RemoteDocumentPlayer(onNamedAction = …)`). Tier-2 is for visual embedding.
 */
@Composable
