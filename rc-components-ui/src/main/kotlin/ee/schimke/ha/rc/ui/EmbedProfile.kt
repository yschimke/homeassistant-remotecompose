...
/**
 * Embed profile for Tier-2 Compose-UI wrappers — AndroidX experimental (FlowRow / FlowLayout ops on
 * for `HaGlance`) with the wrap-content measure feature enabled, so the player measures against
 * parent constraints instead of forcing the document's authored size onto the Compose layout.
 *
 * Mirrors `ee.schimke.ha.rc.androidXExperimentalWrap` from `:rc-converter` — duplicated here so the
 * UI module doesn't pull the converter in.
 */
