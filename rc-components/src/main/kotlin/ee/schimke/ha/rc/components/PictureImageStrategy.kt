 ...
 *
 *  - [Url] — emit the URL form via
 *    `rememberLocalNamedRemoteBitmap(name, url, w, h)`. The player
 *    resolves the URL via the configured `BitmapLoader` at playback.
 *    Use in the **app dashboard** where a Coil-backed loader is
 *    wired.
 *
 *  - [Inline] — emit the bytes form via
 *    `rememberLocalNamedRemoteBitmap(name) { bitmap }` with the
 *    bitmap baked into the doc. Use in **widgets**, where the
 *    runtime has no `BitmapLoader` and the doc must self-contain
 *    every pixel. The host pre-fetches each entity's
 *    `entity_picture` URL before capture and exposes the results
 *    through [Inline.bitmapFor]. Falls back to the card's icon path
 *    when the bitmap isn't ready yet — capture stays synchronous.
 *
...
sealed interface PictureImageStrategy {
  ...
  companion object {
     ...
     * `image_proxy` serves natively for `image.*` entities, and is
     * large enough for camera frames (typically 1680×1080)
     * downscaled to the tile area on the host side.
     */
 ...
 * don't override it get a URL-form doc and the existing dashboard
 * `BitmapLoader` wiring picks the bytes up at playback. Widget
 * hosts must explicitly provide a [PictureImageStrategy.Inline]
 * with pre-fetched bitmaps **inside** the capture composition (a
 * `captureSingleRemoteDocument` content block) — `CompositionLocal`s
 * from the outer tree don't propagate into the capture pass.
 */
