...
/**
 * `custom:ha-bambulab-print_status-card` — printer hero tile with a progress ring + the key sensor
 * readouts (stage, layer, remaining, temperatures).
 *
 * ```
 *   ┌─────────────────────────────────────────────┐
 *   │  P2S                                        │
 ...
 * ```
 *
 * Progress ring is captured statically at the supplied fraction; alpha08 doesn't expose a numeric
 * `RemoteFloat` binding, so live updates re-encode (same constraint as gauge / history-graph).
 */
@Composable
@RemoteComposable
