...
/**
 * The top-level theme toggle. `Material3` is the defaults-only fallback (stock `colorScheme()` +
 * system font, promoted to dynamic color on Android 12+ where available). Every other entry is a
 * curated Terrazzo palette — a named seed colour + a Google-Fonts pairing chosen for a specific HA
 * context.
 *
 * Palette colours are not "seeds" in the materialkolor sense — we don't run a full tonal
 * derivation, we just hand-pick the handful of roles that carry the identity ([TerrazzoPalette]).
 */
enum class ThemeStyle(
  ...
  /**
   * Wall-mounted panels and TV dashboards: max legibility at ≥1m. Uses Atkinson Hyperlegible
   * (designed by the Braille Institute for low-vision distance reading) and a high-contrast teal
   * seed.
   */
  TerrazzoKiosk(
    displayName = "Kiosk",
...
/**
 * Canonical seed colours. Kept here rather than baked into the enum so the lookup is uniform for
 * the default-Material3 case (no seed → dynamic colour or the M3 defaults). Only the Terrazzo
 * entries have a seed.
 */
