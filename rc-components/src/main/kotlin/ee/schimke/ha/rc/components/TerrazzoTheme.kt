 ...
 *
 * Every palette — including Material 3 when a [seedOverride] is supplied
 * — is **derived from its seed** via materialkolor's
 * `dynamicColorScheme(...)`, using the palette's [PaletteStyle] +
 * contrast target. All M3 roles are populated — `tertiary`,
 * `surfaceContainer*`, `inverseSurface`, etc.
 *
 * For [ThemeStyle.Material3], pass a [seedOverride] sampled from the
 * platform wallpaper (Android 8.1+) to get a Material You-style dynamic
 * scheme generated by materialkolor rather than the platform API. When
 * no seed is available the function falls back to the stock
