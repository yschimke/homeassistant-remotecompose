...
/**
 * Per-palette tuning of how [dynamicColorScheme] derives the full Material 3 `ColorScheme` from the
 * seed. The [PaletteStyle] choice is the biggest lever — `Expressive` produces warmer,
 * higher-chroma containers than `TonalSpot`, `Monochrome` strips all hue. [contrast] nudges the
 * WCAG contrast target; Kiosk uses [Contrast.High] since wall-mounted panels need headroom.
 */
 ...
 *
 * Every palette — including Material 3 when a [seedOverride] is supplied — is **derived from its
 * seed** via materialkolor's `dynamicColorScheme(...)`, using the palette's [PaletteStyle] +
 * contrast target. All M3 roles are populated — `tertiary`, `surfaceContainer*`, `inverseSurface`,
 * etc.
 *
 * For [ThemeStyle.Material3], pass a [seedOverride] sampled from the platform wallpaper (Android
 * 8.1+) to get a Material You-style dynamic scheme generated by materialkolor rather than the
 * platform API. When no seed is available the function falls back to the stock `lightColorScheme()`
 * / `darkColorScheme()`.
 */
fun terrazzoColorScheme(
  style: ThemeStyle,
  darkTheme: Boolean,
  seedOverride: Color? = null,
): ColorScheme {
  val seed = seedOverride ?: style.seedColor
  if (seed == null) {
...
/**
 * Build a Material 3 [Typography] scale for the given [style]. Same M3 sizes and line-heights as
 * the defaults — only the font family changes, so a theme switch never reflows the layout.
 */
...
/**
 * Re-map a Material 3 [Typography] so display/headline/title roles use [displayFamily] and
 * body/label roles use [bodyFamily]. The only thing that changes is [TextStyle.fontFamily] — all
 * sizes, weights, and line-heights are the stock M3 defaults.
 */
