 ...
 * ```
 *
 * So every URL-form named bitmap captures a `BitmapData` op with `mImageWidth=1, mImageHeight=1`.
...
/**
 * Bake an [ImageBitmap] into the document under [name]. Mirrors upstream's bytes-form
 * `rememberNamedRemoteBitmap` — the bytes form doesn't suffer from the 1×1 bug (dimensions come
 * from the `ImageBitmap`). Kept here so picture-entity bitmap registration has one local seam
...
/**
 * URL-form named bitmap with explicit [width] / [height]. Replaces upstream's
 * `rememberNamedRemoteBitmap(name, url, domain)` — that version routes through
 * `addNamedBitmapUrl(name, url)` and inherits the 1×1 default. This version threads the dimensions
 * through to `addBitmapUrl(url, w, h)` so the slot has real size, then pairs it with
 * `setNamedVariable(id, …, IMAGE_TYPE)` so the slot is still addressable by name.
 *
 * `RemoteBitmapDecoder.checkBounds` treats the stored width / height as a maximum on URL fetches
 * when `CHECK_DATA_SIZE` is on, so pick a size at least as large as the largest image you expect to
 * load. The tile's draw area is a safe default.
 */
@Composable
fun rememberLocalNamedRemoteBitmap(
  name: String,
...
/**
 * Local replacement for `RemoteComposeWriter.addNamedBitmapUrl(name, url)` that takes explicit
 * dimensions. The upstream method delegates to `addBitmapUrl(url)` which is `addBitmapUrl(url, 1,
 * 1)`; this calls the three-arg `addBitmapUrl(url, w, h)` so the resulting `BitmapData` op carries
 * real dimensions, then names the slot via `setNamedVariable(id, name, IMAGE_TYPE)`.
 * `setNamedVariable` lives on the underlying `RemoteComposeBuffer` (the writer's `getBuffer()`
 * accessor).
 */
