...
/**
 * Watches [WearSyncRepository.slots] + [WearSyncRepository.pinned] and keeps the 10 slot widget
 * services in sync with the user's mobile-side assignments. Both flows are combined so the tier
 * gate (see below) recomputes whenever either changes — pinned-card metadata edits and the
 * `/wear/slots` ↔ `/wear/pinned` arrival race both need a re-apply, not just slot edits. Each slot
 * has two services (small + large) so the system widget picker can filter by container type:
 *
 * - **Empty slot** (`cardKey == ""`) — both Small and Large services for the slot are disabled.
 * - **Assigned slot** — the size choice (Small / Large / Both) decides which subset is enabled.
 *   Disabled services don't appear in the picker, so the user's "Small only" choice on the phone
 *   genuinely hides the large variant.
 * - **Low-data card type** ([WearCardDataTier.SmallOnly]) — large is force-disabled regardless of
 *   `SlotSizePref`. Cards whose identity depends on payloads the wear sync proto can't carry
 *   (forecast, history, calendar events, …) render as an under-filled chip at the large container,
 *   so the picker hides that variant. See `docs/architecture/adaptive-card-layouts.md` § "Wear
 *   data-layer reality".
 *
 * Below [WEAR_WIDGETS_MIN_SDK] every component stays disabled regardless of phone-side assignment;
 * the alpha library short-circuits its own bind path on older Wear OS versions, and an enabled-but-
 * broken component would only confuse the picker.
 *
 * Capability advertisement: when the runtime supports widgets, this controller adds the
 * `terrazzo_wear_widgets` local capability so the phone's [WearCapabilityProbe] can discover us.
 * Removed when the controller stops, so the phone gracefully hides the slot UI again.
 */
class WearSlotsController(
  ...
  fun stop() {
    ...
    // best-effort basis.
    runCatching {
      capabilityClient.removeLocalCapability(CAPABILITY).addOnFailureListener {
        Log.w(TAG, "removeLocalCapability failed", it)
      }
    }
  }
  ...
  private fun applySlots(slots: WearWidgetSlots, pinned: PinnedCardSet) {
    ...
    for (i in 0 until SLOT_COUNT) {
      ...
      val tier =
        slot?.cardKey?.let { typeByKey[it] }?.let { wearCardDataTier(it) } ?: WearCardDataTier.Both
  ...
  private fun applyComponent(pm: PackageManager, component: Class<*>, enabled: Boolean) {
    ...
    runCatching { pm.setComponentEnabledSetting(name, target, PackageManager.DONT_KILL_APP) }
  ...
  private fun advertiseCapability() {
    runCatching {
      capabilityClient.addLocalCapability(CAPABILITY).addOnFailureListener {
        Log.w(TAG, "addLocalCapability failed", it)
      }
    }
  }

  /**
   * Whether this device's runtime supports Glance Wear widgets. Mirrors the alpha library's
   * internal `forceIsAtLeast37ForTesting` test knob — the public companion check is `internal` in
   * alpha09 so we can't call it directly. Bump this constant if the lib settles on a different
   * floor before stable.
   */
  private fun supportsWearWidgets(): Boolean = Build.VERSION.SDK_INT >= WEAR_WIDGETS_MIN_SDK

  /**
   * Mirrors `SlotSizePref` — local helpers so this file doesn't take a hard dep on the proto enum's
   * display strings.
   */
  ...
  companion object {
    ...
    /**
     * Capability advertised when the runtime supports Glance Wear widgets. Mirrored on the phone in
     * `WearCapabilityProbe.CAPABILITY`.
     */
    const val CAPABILITY: String = "terrazzo_wear_widgets"

    /**
     * Minimum SDK_INT where Glance Wear widgets actually render. Inferred from the alpha09 lib's
     * `forceIsAtLeast37ForTesting` static field: production code branches on `>= 37`. Bump if the
     * lib's gate moves.
     */
    ...
    /**
     * Small-container service classes in slot-index order. The list ordering is load-bearing:
     * [applySlots] indexes by position.
     */
    private val SLOT_SMALL_COMPONENTS: List<Class<*>> =
      listOf(
    ...
    private val SLOT_LARGE_COMPONENTS: List<Class<*>> =
      listOf(
