...
/**
 * Bridge `androidx.datastore.core.Serializer<T>` to `kotlinx-serialization-protobuf`. Each Proto
 * DataStore (`/wear/...`) gets one of these.
 *
 * Defensive read: if the file is empty (first read) or corrupt, we return [defaultValue] rather
 * than throwing — DataStore would otherwise propagate as `CorruptionException` and crash the
 * activity.
 */
@OptIn(ExperimentalSerializationApi::class)
class ProtoBufSerializer<T : Any>(private val ksz: KSerializer<T>, override val defaultValue: T) :
  Serializer<T> {
