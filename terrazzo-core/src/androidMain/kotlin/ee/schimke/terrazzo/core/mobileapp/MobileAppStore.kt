...
/**
 * Persists the [MobileAppRegistration] returned by HA for each instance,
 * plus a stable per-install `deviceId` reused across re-registrations so
 * HA keeps recognising us as the same device.
 *
 * The registration blob holds the webhook id and (when encryption is
 * negotiated) the shared secret, so it's stored AES-GCM-encrypted under
 * an AndroidKeyStore key — same pattern as `TokenVault`, separate key
 * alias so the two concerns can be rotated independently.
 */
...
class MobileAppStore(private val context: Context) {
    ...
    /**
     * Returns the deviceId for this instance, generating and persisting a
     * fresh UUID on first call. Stable across re-registration so HA
     * doesn't accrue duplicate mobile_app entries.
     *
     * Stored plaintext: the deviceId is an identifier, not a credential —
     * the webhook id and secret are what need protecting.
     */
    suspend fun getOrCreateDeviceId(instanceId: String): String {
        val prefs = context.store.data.first()
        prefs[deviceIdKey(instanceId)]?.let { return it }
    ...
    suspend fun put(instanceId: String, registration: MobileAppRegistration) {
        val plaintext = json.encodeToString(MobileAppRegistration.serializer(), registration)
