...
/**
 * Per-instance public ("external") URL persisted after we read HA's `get_config` payload. Pairs the
 * URL the user logged in with (their LAN baseUrl) to the public URL HA exposes for remote access —
 * usually a Nabu Casa hostname under `ui.nabu.casa` or a self-hosted reverse proxy.
 *
 * Two access paths:
 * - `Flow` / suspend reads for UI surfaces (Settings).
 * - Synchronous lookup via [externalHostFor] for the OkHttp interceptor that rewrites outgoing
 *   requests to the public host when the LAN destination isn't reachable from the current network.
 *   The interceptor runs on the network thread and can't suspend, so the store keeps a
 *   process-local map mirrored to disk.
 */
...
class RemoteUrlStore(private val context: Context) {
  ...
  /**
   * Persist the external URL HA reports for [baseUrl]. Pass null to clear (the user removed their
   * `external_url` in HA, or sign-out wiped the slot).
   */
  ...
  fun externalUrl(baseUrl: String): Flow<String?> = context.store.data.map { it[keyOf(baseUrl)] }

  suspend fun externalUrlNow(baseUrl: String): String? = context.store.data.first()[keyOf(baseUrl)]

  /**
   * Cached lookup keyed by request host. Returns the parsed `scheme/host/port` for the public URL
   * the interceptor should rewrite to, or null if we have no public URL on file for that host. Runs
   * in O(1) on the network thread.
   */
  fun externalHostFor(internalHost: String): ExternalTarget? = cache[internalHost.lowercase()]
