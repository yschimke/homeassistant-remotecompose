...
/**
 * Holds in-memory overrides applied on top of the captured demo entity state so demo-mode taps
 * produce visible behaviour. Toggles flip the underlying state immediately; cover commands seed an
 * animation that [snapshotOverrides] resolves to a `current_position` per call (one percent every
 * [COVER_ANIM_STEP_MS] ms until it lands at the target).
 *
 * The router is process-scoped — same lifetime as `DemoData` itself. Live HA sessions don't use it;
 * they call HA services directly.
 *
 * Mutations bump [epoch] so observers can invalidate caches keyed on "did anything just change".
 * The app uses this to evict the `CardDocumentCache` so cards baking state into bytes (e.g. the
...
class DemoActionRouter {
  ...
  /**
   * Start (or stop) the open/close animation for [entityId]. The animation is sampled by
   * [snapshotOverrides] so each subsequent frame fed to the dashboard advances the position; on
   * completion the cover settles into its terminal `open` / `closed` state.
   */
  ...
  /**
   * Apply current overrides on top of [base], computed at [nowMs]. Cover animations that have
   * reached their target on this sample are inlined as the terminal state and the in-memory
   * animation is dropped.
   */
  fun snapshotOverrides(base: Map<String, EntityState>, nowMs: Long): Map<String, EntityState> {
  ...
  private data class CoverAnimation(val startMs: Long, val fromPercent: Int, val toPercent: Int) {
    fun sampleAt(nowMs: Long): CoverSample {
      ...
      val clamped = raw.coerceIn(minOf(fromPercent, toPercent), maxOf(fromPercent, toPercent))
...
/**
 * Read the cover position the dashboard last saw, defaulting by-state when the integration doesn't
 * expose `current_position`. Visible to dispatchers so they can seed animations from the latest
 * sample rather than always starting from 0/100.
 */
