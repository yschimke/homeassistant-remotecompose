...
/**
 * Mobile-side wear-widget slot assignments. Five slots (indices 0..4), each pointing to a pinned
 * card by [PinStore]'s `cardKey`. An empty key means the slot is unconfigured — the watch will keep
 * its matching widget provider component disabled so it doesn't appear in the system widget picker.
 *
 * Storage: a single Preferences entry per slot (`slot.0.cardKey` … `slot.4.cardKey`). Keeping the
 * schema flat (one row per slot) matches [ee.schimke.terrazzo.core.widget.WidgetStore] and avoids
 * the JSON-list ceremony that [PinStore] needs for its variable-cardinality data.
 *
 * The store doesn't own the rendering pipeline — it just stores [WearWidgetSlot] rows.
 * [ee.schimke.terrazzo.wearsync.MobileWearSyncManager] subscribes to [slots] and publishes via
 * DataLayer to /wear/slots.
 */
...
data class WearWidgetSlot(
  ...
  val isAssigned: Boolean
    get() = cardKey.isNotEmpty()
}

/**
 * Per-slot Glance Wear container size choice. Wire values match
 * `ee.schimke.terrazzo.wearsync.proto.SlotSizePref` so encoding stays symmetric across modules
...
enum class SlotSize(val wireValue: Int) {
  ...
  val advertisesSmall: Boolean
    get() = this == SmallOnly || this == Both

  val advertisesLarge: Boolean
    get() = this == LargeOnly || this == Both

  companion object {
    fun fromWire(wire: Int): SlotSize = entries.firstOrNull { it.wireValue == wire } ?: Both
  }
}
