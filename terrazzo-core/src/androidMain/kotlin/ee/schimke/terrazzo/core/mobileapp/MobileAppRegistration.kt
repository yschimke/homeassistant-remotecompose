 ...
 *
 * Lives next to the HA refresh token (per `instanceId` = base URL) and is
 * what lets HA target push notifications and Companion-style services at
 * this specific install: HA exposes a `notify.mobile_app_<slug>` service
 * derived from the registered `device_name`.
 *
 * `deviceId` is generated once per app install and reused on every
 * subsequent registration call so HA sees the same device across logins
 * and token refreshes; the rest comes back from HA's response.
 */
@Serializable
