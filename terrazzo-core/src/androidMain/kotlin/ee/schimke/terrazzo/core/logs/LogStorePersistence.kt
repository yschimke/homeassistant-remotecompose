...
/**
 * Persisted form of the [LogStore] ring. Flat shape with a [kind] tag (rather than polymorphic
 * serialisation over the sealed [LogEntry]) because proto-wire polymorphism wants a
 * `SerializersModule` registration we'd otherwise carry just for this one store. Empty strings
 * stand in for nulls — proto3 has no `optional` so a missing field reads as an empty string anyway.
 *
 * Wire numbers are stable; do not reorder. Adding new fields is fine as long as they get a fresh
 * number.
 */
@Serializable
...
internal fun LogEntry.toPersisted(): PersistedLogEntry =
  when (this) {
    is LogEntry.Connection ->
      PersistedLogEntry(
        timestamp = timestamp,
...
internal fun PersistedLogEntry.toModel(): LogEntry? =
  when (kind) {
    KIND_CONNECTION -> {
...
/**
 * Bridges `androidx.datastore.core.Serializer<T>` to `kotlinx-serialization-protobuf`. Defensive
 * read: empty file (first run) or corrupt bytes fall back to [defaultValue] rather than propagating
 * a `CorruptionException` that would crash the activity.
 *
 * Mirrors the same adapter in `app/wearsync/proto/ProtoSerializers.kt` — kept duplicated to avoid
 * pulling :app's wearsync module into :terrazzo-core.
 */
...
internal class ProtoBufStoreSerializer<T : Any>(
  ...
  override suspend fun readFrom(input: InputStream): T {
    ...
    return runCatching { ProtoBuf.decodeFromByteArray(ksz, bytes) }.getOrDefault(defaultValue)
  }
...
@OptIn(ExperimentalSerializationApi::class)
internal inline fun <reified T : Any> protoBufStoreSerializer(
  default: T
): ProtoBufStoreSerializer<T> = ProtoBufStoreSerializer(serializer(), default)
