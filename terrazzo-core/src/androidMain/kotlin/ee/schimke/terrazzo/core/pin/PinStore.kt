...
/**
 * Mobile-side pin store. Source of truth for the user's pinned dashboard items — both individual
 * cards and whole sections — that the watch surfaces as nav destinations and that drive the
 * wear-widget slot picker.
 *
 * Independent of [ee.schimke.terrazzo.core.widget.WidgetStore], which keeps tracking phone
 * home-screen widget installs separately. A user who installs a card as a phone widget does not
 * implicitly pin it for the watch, and vice versa.
 *
 * Storage: a single Preferences DataStore with two JSON-encoded list entries (`pinned_cards` /
 * `pinned_sections`). JSON-in-prefs is enough for the small cardinalities involved (handful of
 * pins) and avoids adding a second DataStore proto.
 *
 * Each pin owns:
 * - a stable `key` (used by [WearWidgetSlots] slot references and as the de-dup token for is-pinned
 *   checks),
 * - an `orderIndex` shared across cards and sections so the user controls the unified Wear
 *   top-level ordering as a single list,
 * - `baseUrl` and dashboard/section provenance so the manage-pinned UI can show "where did this
 *   come from" and the wear sync layer can scope updates to the active session.
 *
 * Section content (the actual card configs the watch renders) is captured into [PinnedCardData] at
 * pin-time and stored alongside the section. This keeps the Wear publisher self-contained and
 * avoids round-tripping the dashboard on every push. If the user edits the section in HA the
 * captured copy goes stale until they re-pin — an acceptable v1 trade.
 */
...
class PinStore(private val context: Context) {
  ...
  /**
   * Pin a card. Idempotent — re-pinning the same key updates the captured payload (e.g. title
   * changed) without changing its order position. New pins land at the end of the unified ordering.
   */
  ...
  /**
   * Reorder the unified top-level list. [keysInOrder] is the desired sequence of keys (cards and
   * sections interleaved); items are reassigned consecutive `orderIndex` values starting at 0.
   * Unknown keys are ignored. Items not present in [keysInOrder] keep their relative order at the
   * tail.
   */
  ...
  private fun Preferences.readCards(): List<MobilePinnedCard> =
    this[CARDS_KEY]?.let {
      runCatching { json.decodeFromString<List<MobilePinnedCard>>(it) }.getOrNull()
    } ?: emptyList()

  private fun Preferences.readSections(): List<MobilePinnedSection> =
    this[SECTIONS_KEY]?.let {
      runCatching { json.decodeFromString<List<MobilePinnedSection>>(it) }.getOrNull()
    } ?: emptyList()

  private fun androidx.datastore.preferences.core.MutablePreferences.writeCards(
    value: List<MobilePinnedCard>
  ) {
    this[CARDS_KEY] = json.encodeToString(value)
  }

  private fun androidx.datastore.preferences.core.MutablePreferences.writeSections(
    value: List<MobilePinnedSection>
  ) {
  ...
  companion object {
    ...
    /**
     * Stable key for a pinned card. Derived from the source-of-truth fields (HA instance +
     * dashboard + raw card config) so that re-pinning the same card from the same dashboard yields
     * the same key — slot assignments survive an unpin / re-pin cycle.
     */
    fun cardKey(baseUrl: String, dashboardUrlPath: String, cardJson: String): String =
      sha16("c|$baseUrl|$dashboardUrlPath|$cardJson")

    /**
     * Stable key for a pinned section. Sections have no inherent id in HA's config so position is
     * the key; if the user reorders sections in HA the key changes and the user has to re-pin.
     */
    fun sectionKey(
      baseUrl: String,
...
/**
 * One pinned card. Captured at pin-time so the wear sync layer doesn't need to re-resolve the
 * dashboard on publish.
 */
@Serializable
...
/**
 * Card snapshot stored inside the pin store. Mirrors the wire shape of `CardSummary` in the
 * wear-sync proto without depending on it (this module sits below the proto's host).
 */
@Serializable
