...
class AlarmKeypadCoordinatorTest {
  ...
  private fun newCoordinator(
    ...
    val coord = AlarmKeypadCoordinator(downstream = sink, scope = scope, idleTimeoutMs = timeoutMs)
  ...
  fun knownLength_flushesImmediatelyOnNthDigit() = runTest {
    ...
    assertTrue(
      sink.captured.isEmpty(),
      "expected no flush before length reached, got ${sink.captured}",
    )
  ...
  @Test
  fun codeLengthZero_skipsBufferingAndFiresImmediately() = runTest {
    val (coord, sink) = newCoordinator(this)
  ...
  fun perEntityState_independent() = runTest {
    ...
    assertEquals(listOf(expectedOther, expectedCall("arm_away", "12")), sink.captured)
  }
}
