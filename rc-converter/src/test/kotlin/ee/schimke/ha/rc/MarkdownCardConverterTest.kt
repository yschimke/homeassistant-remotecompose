...
class MarkdownCardConverterTest {
  ...
  private val snapshot =
    HaSnapshot(
      states =
        mapOf(
          "sensor.living_room_temperature" to EntityState("sensor.living_room_temperature", "21.5"),
  ...
  fun markdownTemplateBindingSupportsMultipleExpressions() {
    ...
    assertEquals("Temp ${haExprToken(0)} / Humidity ${haExprToken(1)}", template.rendered)
  }
  ...
  fun inlineTemplateTokenNextToLinkStaysBound() {
    ...
    val content = "{{ states('sensor.living_room_temperature') }} [history](/lovelace/history)"
    val template = MarkdownCardConverter.MarkdownTemplateBindings.from(content, snapshot)
