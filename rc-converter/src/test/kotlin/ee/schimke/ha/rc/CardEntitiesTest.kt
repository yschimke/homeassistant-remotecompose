...
class CardEntitiesTest {
  ...
  private fun rawConfig(json: String): JsonObject = Json.parseToJsonElement(json) as JsonObject
  ...
  fun entitiesArrayOfStrings() {
    val ids =
      cardEntityIds(
        card("""{"entities":["sensor.temp","sensor.humidity"]}""", type = "history-graph")
      )
  ...
  fun nestedConditionalAndStack() {
    ...
    assertEquals(setOf("binary_sensor.door", "light.kitchen", "sensor.temp"), ids)
  }
  ...
  fun snapshotBindingsSkipMissingEntities() {
    ...
    val snapshot = HaSnapshot(states = mapOf("light.kitchen" to EntityState("light.kitchen", "on")))
    val bindings = cardSnapshotBindings(setOf("light.kitchen", "sensor.missing"), snapshot)
  ...
  fun pictureEntityIdsTopLevel() {
    val ids =
      cardPictureEntityIds(
        card("""{"type":"picture-entity","entity":"camera.front"}""", type = "picture-entity")
      )
  ...
  fun pictureEntityIdsSkipsNonPictureCards() {
    ...
    val ids =
      cardPictureEntityIds(card("""{"type":"tile","entity":"camera.front"}""", type = "tile"))
    assertTrue(ids.isEmpty())
  }
  ...
  fun pictureEntityIdsSkipsEntitiesWithoutDomain() {
    ...
    val ids =
      cardPictureEntityIds(
        card("""{"type":"picture-entity","entity":"front"}""", type = "picture-entity")
      )
    assertTrue(ids.isEmpty())
  }
}
