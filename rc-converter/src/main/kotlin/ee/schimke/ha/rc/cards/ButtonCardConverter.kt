 ...
 * so tap crossfades the accent when the host writes the new state back;
 * read-only entities go through the simpler [RemoteHaButton].
 *
 * In [CardSizeMode.Fixed] the converter encodes a 2-tier ladder driven
 * by the runtime canvas (mirrors the button ladder in
 * `docs/architecture/adaptive-card-layouts.md` §button):
 *
 *   * **icon + name** — the full button, self-centred via `fillMaxSize`
 *     so it fills / centres rather than pinning top-left on big cells.
 *   * **icon only** — [RemoteHaButtonIcon], the tinted-chip identity
 *     tier for cells too small for the label (`1×1` launcher, Wear S).
 *
 * The gate is a **single** boolean off a single float expression
 * `min(w, h · minW/minH).ge(minW)`, which is true only when the cell is
 * both wide enough (`w ≥ minW`) and tall enough (`h ≥ minH`). Encoding
 * the 2-D test as one `min` expression (rather than `w.ge(..)
 * .and(h.ge(..))`) sidesteps the alpha010 quirk where `RemoteBoolean
 * .and()` and nested width/height state-layouts collapse to `false` at
 * playback (#224, see `GaugeCardConverter`). It lets the narrow-tall
 * `1×1` cell and the wide-short Wear S chip *both* fall to the icon-only
 * tier while the wide-and-tall `2×1` / `3×2` / Wear L cells keep the
 * name — something no single width-only or height-only gate can do.
 */
class ButtonCardConverter : CardConverter {
    ...
    private fun AdaptiveButton(data: HaButtonData, modifier: RemoteModifier) {
        ...
        RemoteBox(modifier = modifier.fillMaxSize()) {
            ...
            RemoteText(
                text = gate.toRemoteString(IntFormat),
