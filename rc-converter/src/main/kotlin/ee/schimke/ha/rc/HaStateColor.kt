...
/**
 * Approximation of HA's `stateColor(entity)` used by `hui-tile-card` (see
 * `home-assistant/frontend/src/common/entity/state_color.ts`).
 *
 * Semantics live on the typed [HaEntity] hierarchy — exhaustive `when` instead of scattered string
 * matching.
 *
 * Three flavors:
 * - [resolve]: static — return the current state's color. Good when the tile author wants a frozen
 *   snapshot and the host re-renders on state change.
 * - [activeFor]: the "active" color regardless of current state. Paired with [inactiveFor] behind
 *   an `isOn.select()` so the RC document reflects state changes at playback.
 * - [inactiveFor]: the "off" / "alert" color, also independent of current state.
 */
object HaStateColor {
  ...
  fun resolve(entity: EntityState?): Color {
    ...
    return when (typed) {
      ...
      is HaEntity.Person,
      is HaEntity.DeviceTracker -> if (typed.isActive == true) Color(0xFF4CAF50) else GrayOff
  ...
  fun activeFor(entity: EntityState?): Color {
    ...
    return when (typed) {
      ...
      is HaEntity.Switch,
      is HaEntity.InputBoolean -> Color(0xFF2196F3)
      ...
      is HaEntity.Weather -> Color(0xFF0288D1)
      is HaEntity.Person,
      is HaEntity.DeviceTracker -> Color(0xFF4CAF50)
  ...
  private fun climateColor(mode: ClimateMode): Color =
    when (mode) {
      is ClimateMode.Heat,
      is ClimateMode.HeatCool -> Color(0xFFE65100)
      ...
      is ClimateMode.Off,
      is ClimateMode.Unavailable,
      is ClimateMode.Unknown -> GrayOff
    }

  private fun sensorByDeviceClass(deviceClass: String?): Color? =
    when (deviceClass) {
      "temperature" -> Color(0xFF2196F3)
      "humidity",
      "moisture" -> Color(0xFF0288D1)
      "battery" -> Color(0xFF43A047)
      "power",
      "energy",
      "current",
      "voltage" -> Color(0xFFFFA000)
      "illuminance" -> Color(0xFFFBC02D)
      "pressure" -> Color(0xFF5E35B1)
      "motion",
      "occupancy",
      "presence" -> Color(0xFFE65100)
      "door",
      "window",
      "opening",
      "garage" -> Color(0xFF00796B)
