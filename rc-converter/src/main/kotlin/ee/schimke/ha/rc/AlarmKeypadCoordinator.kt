...
/**
 * Accumulates [HaAction.AlarmKey] presses per entity and fires the corresponding
 * `alarm_control_panel.alarm_*` service on [downstream] once it decides the user has finished
 * entering an attempt.
 *
 * ## Timing model
 *
 * Each keypad key on the alarm panel is its own host action — the .rc document does not buffer
 * anything. This coordinator owns the buffer on the host side, paired with the most recent
 * [HaAction.AlarmIntent] (which the ARM AWAY / ARM HOME / DISARM buttons emit). It dispatches once
 * one of these "complete attempt" conditions fires:
 *
 * - **Known length (codeLength > 0):** flush the moment the buffer reaches that length.
 * - **No code required (codeLength = 0):** flush the intent immediately on press.
 * - **Unknown length (codeLength = null):** flush after [idleTimeoutMs] of no further key presses
 *   while an intent is pending.
 * - **Intent after typing:** if the user types digits first then taps ARM, flush immediately with
 *   whatever's in the buffer.
 *
 * Other [HaAction] variants pass through to [downstream] unchanged so this can sit transparently in
 * front of the dashboard's dispatcher.
 */
