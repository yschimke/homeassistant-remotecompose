...
object HaIconMap {
  ...
  fun resolve(iconOverride: String?, entity: EntityState?): ImageVector {
    ...
    entity?.attributes?.get("icon")?.jsonPrimitive?.content?.let {
      mdi(it)?.let { v ->
        return v
      }
    }
    val typed = entity?.toTyped() ?: return Icons.AutoMirrored.Filled.Help
    if (typed is HaEntity.Other)
      byDomain(entity.entityId)?.let {
        return it
      }
  ...
  /**
   * Domain-only fallback for entity classes the typed hierarchy doesn't model (camera, scene,
   * script, …). Keeps `mdi:` overrides authoritative.
   */
  private fun byDomain(entityId: String): ImageVector? =
    when (entityId.substringBefore('.')) {
      "camera" -> Icons.Filled.Videocam
      else -> null
    }

  private fun HaEntity.defaultIcon(): ImageVector =
    when (this) {
      is HaEntity.Light -> Icons.Filled.Lightbulb
      is HaEntity.Switch,
      is HaEntity.InputBoolean -> Icons.Filled.PowerSettingsNew
      ...
      is HaEntity.Cover ->
        when (deviceClass) {
          "door",
          "garage" -> Icons.Filled.DoorFront
          "window" -> Icons.Filled.Window
          "shade",
          "blind",
          "curtain" -> Icons.Filled.BlindsClosed
          else -> Icons.Filled.Window
        }
      is HaEntity.Lock ->
        if (state is LockState.Unlocked) Icons.Filled.LockOpen else Icons.Filled.Lock
      ...
      is HaEntity.Sensor ->
        when (deviceClass) {
          ...
          "power",
          "energy" -> Icons.Filled.ElectricBolt
      ...
      is HaEntity.BinarySensor ->
        when (deviceClass) {
          "motion",
          "occupancy" -> Icons.Filled.SensorOccupied
          "door" -> Icons.Filled.SensorDoor
          "connectivity" -> Icons.Filled.Wifi
          else -> Icons.Filled.Devices
        }
      is HaEntity.Person,
      is HaEntity.DeviceTracker -> Icons.Filled.Mood
  ...
  private val MdiMap: Map<String, ImageVector> =
    mapOf(
