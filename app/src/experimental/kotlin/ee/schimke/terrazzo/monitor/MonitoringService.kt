...
/**
 * User-initiated bounded monitoring window for a single card. Runs as a `dataSync` foreground
 * service — no UI surface of its own; the only visible artefact is an ongoing notification whose
 * `CustomBigContentView` is a `RemoteViews` playing the card's RC `DrawInstructions` at the current
 * snapshot. Each tick re-encodes the document with fresh state so values drift in place, and kicks
 * `ACTION_APPWIDGET_UPDATE` so any pinned instance of the same card tracks the same cadence.
 *
 * Scope today: demo mode only — the service pulls snapshots from [DemoData]. Live mode needs a
 * shared non-UI HA session; landing in a follow-up.
 *
 * Invariant: the window is bounded. The service self-stops after [EXTRA_DURATION_MINUTES]; nothing
 * here can run forever.
 */
class MonitoringService : Service() {
  ...
  override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
    ...
    val card = CardConfig(type = cardType, raw = json.parseToJsonElement(cardJson) as JsonObject)
  ...
  private suspend fun runMonitoringLoop(card: CardConfig, durationMinutes: Int) {
    ...
    while (scope.isActive && System.currentTimeMillis() < deadline) {
      runCatching { emitTick(card, deadline) }.onFailure { Log.w(TAG, "tick failed", it) }
  ...
  private fun placeholderNotification(card: CardConfig, durationMinutes: Int): Notification =
    baseBuilder(card).setContentText("Starting $durationMinutes-minute window…").build()
    ...
    fun start(context: Context, card: CardConfig, durationMinutes: Int = DEFAULT_DURATION_MINUTES) {
      val intent =
        Intent(context, MonitoringService::class.java).apply {
          ...
          putExtra(EXTRA_CARD_JSON, Json.encodeToString(JsonObject.serializer(), card.raw))
