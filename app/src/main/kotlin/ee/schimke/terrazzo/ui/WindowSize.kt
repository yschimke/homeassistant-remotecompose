 ...
 * [ee.schimke.ha.rc.CardWidthClass.Compact] cards into a single row. Thresholds match the Material
 * 3 window size class breakpoints (`Compact <600dp`, `Medium 600..839dp`, `Expanded ≥840dp`) — same
 * numbers as `WindowSizeClass`, without the extra dependency surface.
 */
enum class WindowSize {
  ...
  companion object {
    fun fromWidthDp(widthDp: Int): WindowSize =
      when {
        widthDp < 600 -> Compact
        widthDp < 840 -> Medium
        else -> Expanded
 ...
 *
 * @property compactCardsPerRow how many [CardWidthClass.Compact] cards fit in one row at this
 *   width. Two on a phone is enough to make a typical "lights / cover / scene" cluster usable;
 *   tablets and foldables get more so a dashboard authored for the wall display doesn't waste the
 *   extra real estate.
 * @property maxContentWidth caps the dashboard column on very wide screens so a 1280-dp tablet
 *   doesn't render a 1280-dp-wide `entities` list — beyond ~720 dp the list becomes unscannable.
 *   `Dp.Unspecified` means "no cap". Only applied when the dashboard has no sections (legacy
 *   `view.cards`); section columns enforce their own width via [maxSectionColumns].
 * @property horizontalGutter outer padding either side of the dashboard column. Bigger on tablets
 *   purely for breathing room.
 * @property maxSectionColumns ceiling on how many HA sections render side-by-side in wide mode. The
 *   actual column count is `min(maxSectionColumns, sectionCount)`. `1` means "always single-column"
 *   — sections still render with their titles, just stacked vertically.
 */
