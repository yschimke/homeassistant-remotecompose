...
/**
 * Bottom sheet shown when the user long-presses a card. Live preview on top (rendered through
 * `CachedCardPreview` with the same [widgetsProfile] profile the home screen widget uses, so what
 * you see is what gets installed), then an "Add to Home Screen" button that calls
 * `requestPinAppWidget`.
 *
 * `CachedCardPreview` (vs. `RemotePreview`) buys two things here:
 * - It pushes named-binding writes into the running player whenever [snapshot] changes, so the
 *   preview tracks live entity state instead of freezing on the first frame.
 * - It captures once per `(card, theme, profile)` and keeps playing, so opening the sheet
 *   repeatedly for the same card is cheap.
 *
 * Install cap: disable the button once five widgets are already installed. The cap-check number
 * comes live from [WidgetStore] so removing one in Settings re-enables the button without a
 * restart.
 */
...
fun WidgetInstallSheet(
  ...
  val previewSize =
    remember(card, snapshot) { WidgetSizing.forPreview(registry.cardHeightDp(card, snapshot)) }
  ...
  val pinKey =
    remember(baseUrl, dashboardUrlPath, cardData.rawJson) {
  ...
  ModalBottomSheet(onDismissRequest = onDismiss, sheetState = sheetState) {
    Column(
      ...
      OutlinedButton(
        ...
        Text(text = if (isCardPinned) "  Unpin from Wear" else "  Pin to Wear")
      }
...
/**
 * Cache key for the install-sheet preview. Mirrors the dashboard's `CardSlotCacheKey` shape —
 * snapshot is deliberately omitted so live entity changes flow through named bindings instead of
 * forcing a re-encode (see `CachedCardPreview` for the binding push). Profile is part of the key so
 * the same card cached for the dashboard (AndroidX-experimental) doesn't collide with the widget
 * capture (V7).
 */
private data class WidgetPreviewCacheKey(val card: CardConfig, val theme: HaTheme, val profile: Any)
