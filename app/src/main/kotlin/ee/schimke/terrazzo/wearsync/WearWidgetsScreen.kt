...
/**
 * Mobile screen for configuring the 5 wear-widget slots. Each slot holds a reference to a pinned
 * card (by [MobilePinnedCard.key]); the watch enables one widget provider per assigned slot, and
 * the system widget picker on the watch only surfaces the enabled ones.
 *
 * This screen is reachable from the dashboard top-bar overflow menu — but only when
...
fun WearWidgetsScreen(onBack: () -> Unit) {
  ...
  Scaffold(
    ...
    Column(modifier = Modifier.fillMaxSize().padding(padding).padding(horizontal = 16.dp)) {
      Text(
        text =
          "Each slot maps to one widget provider on your watch. " +
...
private fun SlotRow(
  ...
  Card(modifier = Modifier.fillMaxWidth()) {
      ...
      Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
          ...
          Text(
            text =
              when {
                assignedCard != null -> assignedCard.card.title.ifEmpty { assignedCard.card.type }
                slot.isAssigned -> "Assigned card no longer pinned"
                else -> "Unassigned"
              },
            style = MaterialTheme.typography.bodySmall,
            color =
              if (assignedCard != null) MaterialTheme.colorScheme.primary
    ...
    DropdownMenu(expanded = pickerOpen.value, onDismissRequest = { pickerOpen.value = false }) {
