...
/**
 * Settings → Diagnostics. Mounted from the existing settings screen via a small "Sync diagnostics"
 * link, so the wear data-layer chatter is visible to power users who want to make sure messages
 * aren't being fired too often.
 *
 * Surfaces:
 * - the cumulative DataItem write / MessageClient send counts
 * - the timestamp of the last lease arrival (which decides whether phone is currently streaming or
 *   batching)
...
internal fun SyncDiagnosticsContent(
  ...
  Scaffold(
    ...
    Column(
      ...
      Text(text = "What the watch sees", style = MaterialTheme.typography.titleMedium)
      Text(
        text =
          "Phone publishes wear data via DataClient (cold reads) and " +
      ...
      Row(
        ...
        Column {
          ...
          Text(
            ...
            color =
              if (streamActive) MaterialTheme.colorScheme.primary
      ...
      StatRow(
        "Lease window",
        if (stats.lastLeaseAtMs == 0L) "never"
        else
          relativeNow(stats.lastLeaseAtMs) + " (window: ${WearSyncPaths.LEASE_WINDOW_MS / 1000}s)",
      )
      StatRow(
        "Last DataItem write",
...
/**
 * Format the rolling send window as either a per-minute rate (when we have ≥ 2 timestamps) or a
 * count. The window is `recentSendMs.size` timestamps; their span in seconds is `(last - first) /
 * 1000`.
 */
